...
async function addTraining() {
  ...
  if (!date) {
    return showToast("Bitte ein Datum auswählen.");
  }
...
async function addPlayer() {
  ...
  if (!name) {
    return showToast("Bitte einen Namen eingeben.");
  }

  if (data.players.length >= 25) {
    return showToast("Maximal 25 Spieler.");
  }
...
async function deletePlayer(id) {
  ...
  if (
    !confirm(`"${player.name}" wirklich löschen?`)
  ) {
    return;
  }
...
async function setEntry(playerId, key, value) {
  const training = data.trainings.find(
  ...
  if (key === "attendance") {
    ...
    entry.cancellation = entry.cancellation === "short" ? "none" : "short";
    entry.attendance = entry.cancellation === "short" ? 0 : 1;
...
function calculatePlayer(playerId) {
  ...
  data.trainings.forEach(training => {
    ...
    // Normale Abwesenheit:
    // keine Punkte, aber auch kein Abzug
    if (entry.attendance !== 1) {
      return;
    }

    // Spieler war anwesend
    presentCount++;
...
function renderTrainingSelect() {
  ...
  document.getElementById(
    ...
    training
      ? `Ausgewählt: ${formatDate(training.date)}`
      : "";
}
...
async function init() {
  ...
  // Sicherheitshalber alte/fehlende
  // Datenstrukturen korrigieren
